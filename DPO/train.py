...
if __name__ == "__main__":
   ...
   labels = torch.LongTensor(
       [[-100] * len(prompt_ids) + good_response_ids] + [[-100] * len(prompt_ids) + bad_response_ids for bad_response_ids in bad_response_ids_list]
   )[:, 1:]
   ...
   # 计算loss，会自动进行广播
   logits_diff = (policy_good_logps - reference_good_logps) - (policy_bad_logps - reference_bad_logps)
